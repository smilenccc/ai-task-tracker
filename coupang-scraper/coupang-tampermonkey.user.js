   ...
   > 3; });
              ...
              if (name || priceMatch) {
                orders.push({
                  ...
                  status: status, productLink: link ? link.href : '', imageUrl: img ? img.src : ''
                });
              }
            });
          } else {
            ...
            orders.push({
              orderId: 'coupang-' + date + '-' + idx, date: date, name: name,
              price: priceMatch ? parseInt(priceMatch[1].replace(/,/g, '')) : 0,
              quantity: qtyMatch ? parseInt(qtyMatch[1]) : 1, status: status
            });
          }
        } catch (e) {
      ...
      return new Promise(function(resolve, reject) {
        GM_xmlhttpRequest({
          ...
          onload: function(response) {
            try { var result = JSON.parse(response.responseText); resolve(result); }
    ...
    function main() {
      waitForLoad().then(function() {
        ...
        if (orders.length === 0) { console.log('[Coupang 提取] 未找到訂單'); return; }
        sendToServer(orders).then(function(result) {
          console.log('[Coupang 提取] Server 回應:', result);
          showNotification('已同步 ' + result.added + ' 筆新訂單', '共 ' + result.total + ' 筆訂單已儲存');
        }).catch(function(err) {
          console.warn('[Coupang 提取] 無法連線到 Server:', err);
          showNotification('無法連線到分析 Server', '請確認 server 已啟動：node coupang/server.mjs');
        });
      });
    }
