  ...
  function waitForLoad() {
    return new Promise(resolve => {
      if (document.readyState === 'complete') {
        setTimeout(resolve, 3000); // 額外等 3 秒讓動態內容載入
      } else {
    ...
    cards.forEach((card, idx) => {
      try {
        ...
        if (products.length > 0) {
          ...
          const qtyMatch = text.match(/(\d+)\s*件/);
          const lines = text.split('\n').map(l => l.trim()).filter(l => l.length > 3);
          const name = lines.find(l =>
            !l.match(/^\d{4}\./) && !l.match(/^\$/) && !l.match(/^\d+\s*件/) &&
  ...
  function showNotification(title, text) {
    ...
    div.innerHTML = `
      <div style="position:fixed;top:20px;right:20px;z-index:99999;background:#1a1d28;color:#e4e4e7;
        ...
        font-family:-apple-system,sans-serif;font-size:14px;max-width:350px;animation:slideIn 0.3s ease">
